...
from dataset.ham_loader import han_loader
...
def parse_option():
    ...
    parser.add_argument('--hidden', type=int, default=256, help='num_workers')
    parser.add_argument('--epochs', type=int, default=100, help='number of training epochs')
    parser.add_argument('--gpu_id', type=str, default='240', help='id(s) for CUDA_VISIBLE_DEVICES')
    ...
    # dataset
    parser.add_argument('--model', type=str, default='resnet34_cifar')
    parser.add_argument('--dataset', type=str, default='cifar10', choices=['cinic', 'cifar10', 'cifar100', 'imagenet', 'isic'], help='dataset')
...
def main_worker(gpu, ngpus_per_node, opt):
    ...
    print(opt.multiprocessing_distributed)
    try:
        if 'scale' in opt.model:
            model = model_dict[opt.model](num_classes=n_cls, hidden=opt.hidden)
        else:
    ...
    if opt.dataset == 'cifar10':
        ...
        train_loader, val_loader = get_cinic_dataloaders(opt)
    elif opt.dataset == 'cifar100':
        train_loader, val_loader = get_cifar100_dataloaders(batch_size=opt.batch_size, num_workers=opt.num_workers)
    elif opt.dataset == 'isic':
        train_loader, val_loader = han_loader(batch_size=opt.batch_size, num_workers=opt.num_workers, split_ratio=0.9)
    ...
    for epoch in range(1, opt.epochs + 1):
        ...
        if not opt.multiprocessing_distributed or opt.rank % ngpus_per_node == 0:
            ...
            save_file = os.path.join(opt.save_folder, '{}_current.pth'.format(opt.model))
            ...
            save_dict_to_json(test_merics, os.path.join(opt.save_folder, "test_current_metrics.json"))
    ...
    if not opt.multiprocessing_distributed or opt.rank % ngpus_per_node == 0:
        ...
        f = open('./record_vanilla', 'a')
        f.write(opt.model + ':' + str(best_acc))
        f.write('\n')
        f.close()
