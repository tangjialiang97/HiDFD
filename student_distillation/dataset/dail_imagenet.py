...
from dataset.base import DALIDataloader
...
IMG_DIR = '/opt/data/common/Data_set/ILSVRC2012/imagenet'
TRAIN_BS = 512
TEST_BS = 200
NUM_WORKERS = 4
VAL_SIZE = 256
...
def get_imagenet_loader(opt):
    pip_train = HybridTrainPipe(batch_size=opt.batch_size, num_threads=opt.num_workers, device_id=0, data_dir=IMG_DIR + '/train',
                                crop=CROP_SIZE, world_size=1, local_rank=0)
    pip_test = HybridValPipe(batch_size=TEST_BS, num_threads=opt.num_workers, device_id=0, data_dir=IMG_DIR + '/val',
                             crop=CROP_SIZE, size=VAL_SIZE, world_size=1, local_rank=0)
    train_loader = DALIDataloader(pipeline=pip_train, data_size=IMAGENET_IMAGES_NUM_TRAIN, batch_size=TRAIN_BS,
                                  onehot_label=True)
    test_loader = DALIDataloader(pipeline=pip_test, data_size=IMAGENET_IMAGES_NUM_TEST, batch_size=TEST_BS,
                                 onehot_label=True)
...
#     train_loader = DALIDataloader(pipeline=pip_train, data_size=IMAGENET_IMAGES_NUM_TRAIN, batch_size=TRAIN_BS, onehot_label=True)
#     test_loader = DALIDataloader(pipeline=pip_test, data_size=IMAGENET_IMAGES_NUM_TEST, batch_size=TEST_BS, onehot_label=True)
