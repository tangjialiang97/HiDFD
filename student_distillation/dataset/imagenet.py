...
def get_data_folder(dataset='imagenet'):
    ...
    """
    data_folder = os.path.join('/opt/data/private/data/', dataset)
...
class ImageNetDataset(Dataset):
    def __init__(self, root_dir, transform=None):
        ...
        # 遍历文件夹和子文件夹，获取图像路径和标签
        for folder_name in os.listdir(root_dir):
            folder_path = os.path.join(root_dir, folder_name)
            if os.path.isdir(folder_path):
                for image_name in os.listdir(folder_path):
                    image_path = os.path.join(folder_path, image_name)
                    self.image_paths.append((image_path, folder_name))
...
def get_imagenet_loader(opt):
    # 定义数据集的文件路径
    train_path = '/opt/data/common/Data_set/ILSVRC2012/imagenet/train'
    test_path = '/opt/data/common/Data_set/ILSVRC2012/imagenet/val'
    ...
    train_set = ImageFolderSample(train_path, transform=train_transform)
