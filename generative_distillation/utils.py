...
from get_i32_dataloader import get_i32_dataloader
...
def prepare_parser():
    ...
    ###multiprocess###
    parser.add_argument(
        '--multiprocessing_distributed', action='store_true', default=False)
    parser.add_argument('--dist_url', default='tcp://127.0.0.1:23451', type=str,
    ...
    parser.add_argument(
        ...
        help='Which Dataset to train on, out of I128, I256, C10, C100;'
             ...
             '(default: %(default)s)')
    parser.add_argument(
        '--train_num', type=int, default=None)
    parser.add_argument(
        '--select_dir', type=str, default=None)
    parser.add_argument(
    ...
    parser.add_argument(
        '--D_reg_lambda', type=float, default=1.0,
        help='Classification task lambda coefficient for D (default: %(default)s)')
    parser.add_argument(
        '--G_ba_lambda', type=float, default=1.0,
        help='Classification task lambda coefficient for D (default: %(default)s)')
    parser.add_argument(
        '--G_reg_lambda', type=float, default=1.0,
        help='Classification task lambda coefficient for D (default: %(default)s)')
...
imsize_dict = {'I32': 32, 'I32_hdf5': 32,
               ...
               'C10_select': 32, 'C100_select': 32, 'CINIC_select': 32, 'TI200_select': 64}
root_dict = {'I32': 'ImageNet', 'I32_hdf5': 'ILSVRC32.hdf5',
             ...
             'C10_select': 'cifar10_select', 'CINIC_select': 'cifar10_select', 'C100_select': 'cifar100_select',
             'TI200_select': 'web_tiny_imagenet_64'}
nclass_dict = {'I32': 1000, 'I32_hdf5': 1000,
               ...
               'C10_select': 10, 'C100_select': 100, 'CINIC_select': 10, 'TI200_select': 200}
...
classes_per_sheet_dict = {'I32': 50, 'I32_hdf5': 50,
                          ...
                          'C10_select': 10, 'CINIC_select': 10, 'C100_select': 100, 'TI200_select': 100}
...
def get_data_loaders(dataset, data_root=None, augment=False, batch_size=64,
                     ...
                     num_epochs=500, use_multiepoch_sampler=False, multiprocessing_distributed=False, rank=0,
                     world_size=1, train_num=None, select_dir=None,
                     **kwargs):
    # Append /FILENAME.hdf5 to root if using hdf5
    data_root += '/%s' % root_dict[dataset]
    # data_root = '/opt/data/private/data/tiny-imagenet-200/train'
    print('Using dataset root location %s' % data_root)

    # if dataset in ['C10_select', 'C100_select']:
    #   which_dataset = get_i32_dataloader()
    # else:
    if dataset not in ['C10_select', 'C100_select', 'CINIC_select']:
    ...
    if dataset in ['C10_select', 'C100_select', 'CINIC_select']:
        train_set = get_i32_dataloader(transform=train_transform, dataset=dataset, train_num=train_num,
...
def generate_images(G, num_images, G_batch_size, save_dir, experiment_name, dataset):
    print('Generating %d images from model...' % num_images)
    G.eval()
    iter_num = num_images // G_batch_size + 1
    if not os.path.exists('%s/%s' % (save_dir, experiment_name)):
        os.makedirs('%s/%s' % (save_dir, experiment_name))
    for idx in range(iter_num):
        # z_, y_ = prepare_z_y(G_batch_size, G.dim_z, 10, device='cuda', fp16=False)
        z_ = torch.randn(G_batch_size, G.dim_z, device='cuda')
        if dataset == 'C10' or dataset == 'C10_select':
            y_ = torch.randint(0, 10, (G_batch_size,), device='cuda')
        elif dataset == 'C100' or dataset == 'C100_select':
            y_ = torch.randint(0, 100, (G_batch_size,), device='cuda')
        elif dataset == 'CINIC' or dataset == 'CINIC_select':
            y_ = torch.randint(0, 10, (G_batch_size,), device='cuda')
        elif 'TI200' in dataset:
            y_ = torch.randint(0, 200, (G_batch_size,), device='cuda')
        # y_ = torch.zeros(G_batch_size, 10, device='cuda')
        with torch.no_grad():
            imgs = G(z_, G.shared(y_))
            # torchvision.utils.save_image(imgs.float().cpu(), '%s/%s/%d.png' % (save_dir, experiment_name, idx),
            #                              nrow=int(imgs.shape[0] ** 0.5), normalize=True)

            # 保存每个生成的图像
            for i, img in enumerate(imgs):
                # 获取当前图像的标签描述
                label_desc = y_[i].item()

                # 为当前标签创建子文件夹（如果尚不存在）
                label_dir = os.path.join(save_dir, experiment_name, str(label_desc))
                os.makedirs(label_dir, exist_ok=True)

                # 保存图像
                img_path = os.path.join(label_dir, f'image_{idx}_{i}.png')
                torchvision.utils.save_image(img, img_path, normalize=True)


if __name__ == '__main__':
    z_, y_ = prepare_z_y(G_batch_size=10, dim_z=128, nclasses=10, device='cuda', fp16=False)
    y_.sample_()
    print(z_)
    print(y_)
    print(y_ * 2 + 1)